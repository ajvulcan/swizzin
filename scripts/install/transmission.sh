...
# Adapted to SERVIDOR HD by ajvulcan
...
_start_transmission () {
    #Esto necesita ser hecho siempre solo después de que las configuraciones se hayan hecho, de otra forma transmission las sobreescribe.
...
echo "Transmission RPC puerto para ${user} = ${rpc_port}"
...
if [[ -n $1 ]]; then
	user=$1
    echo "Configurando transmission para $user"
	_mkdir_transmission
    _mkconf_transmission
    _nginx_transmission
	exit 0
...
echo "Creando directorios para $user"
_mkdir_transmission
echo "Creando configuracion para $user"
_mkconf_transmission
_start_transmission
done
