#!/bin/bash
#
# [Servidor HD :: Install Subsonic package]
#
# by ajvulcan
#
# Servidor HD
...
echo "Creando directorio temporal de instalacion subsonic-tmp ... "
mkdir /root/subsonic-tmp

echo "Descargando las dependencias de Subsonic e instalando ... "
case $codename in
  "buster")
  echo "Añadiendo repositorio adoptopenjdk"
  ...
  apt-get -y -q install adoptopenjdk-8-hotspot >>"${OUTTO}" 2>&1 || { echo "ERROR: No se pudo descargar java. Saliendo."; exit 1; }
  ;;
  *)
  apt-get -y install openjdk-8-jre || { echo "ERROR: No se pudo descargar java. Saliendo."; exit 1; }
  ;;
esac
...
wget -qO /root/subsonic-tmp/subsonic.deb $latest || { echo "No se pudo descargar Subsonic. Saliendo."; exit 1; }
cd /root/subsonic-tmp
dpkg -i subsonic.deb >>"${OUTTO}" 2>&1

touch /install/.subsonic.lock

echo "Borrando el directorio de instalación subsonic-tmp ... "
cd
rm -rf /root/subsonic-tmp

echo "Modificando el script de arranque para Subsonic ... "
...
echo "Habilitando configuración systemd para Subsonic"
...
echo "¡Instalación de Subsonic completada!"