#!/bin/bash
#
# [Servidor HD :: Install pyLoad package]
#
# Servidor HD by ajvulcan
#
...
echo "Instalando dependencias necesarias para pyLoad ... "
...
for depend in $LIST; do
  apt-get -qq -y install $depend >>"${log}" 2>&1 || { echo "ERROR: APT-GET no pudo instalar una dependencia requerida: ${depend}. Mal rollo..."; }
done
...
if [ -f "/opt/pyload/files.db" ]; then
  ...
  echo "Algo fué mal en la configuracion de usuario -- no te vas a poder logear"
fi
...
echo "Habilitando y lanzando servicios de pyLoad ... "
